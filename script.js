...
async function showInstructions() {
  ...
  await typeLine("📚 Пример: зашифруем слово MOON с ключом SUN");
  await typeLine("MOON → M=12, O=14, O=14, N=13");
  await typeLine("SUN → S=18, U=20, N=13 → повторяем: SUNS");

  await typeLine("📐 Шифруем по формуле: (Исходная + Ключ) mod 26");
  await typeLine("   M(12) + S(18) = 30 → 4 → E");
  await typeLine("   O(14) + U(20) = 34 → 8 → I");
  await typeLine("   O(14) + N(13) = 27 → 1 → B");
  await typeLine("   N(13) + S(18) = 31 → 5 → F");
  await typeLine("→ Зашифрованное слово: EIBF");

  await typeLine("🔑 Теперь вернёмся к твоей загадке:");
